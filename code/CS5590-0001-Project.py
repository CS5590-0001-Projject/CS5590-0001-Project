...
val_data=MyDataset(txt='/home/molan/Desktop/project/val_new.txt', transform=transform)
...
# evaluation: clalulate precision, recall, accuracy, and F1-score.
feature_model = torch.load('/home/molan/Desktop/project/feature_extraction_model_triple.h5')
classidication_model = torch.load('/home/molan/Desktop/project/classification_model_triple.h5')
feature_model.eval()
classidication_model.eval()
with torch.no_grad():
    for data in test_loader:
       ...
       target.append(label_name[an_name[0]])
target = np.asarray(target)
target = torch.Tensor(target).type(torch.LongTensor)
predict = np.asarray(predict)
predict = torch.Tensor(predict).type(torch.LongTensor)
confusion_matrix =torch.zeros(5749,5749)
...
print('recall:',torch.mean(recall))
print('precision: ',torch.mean(precision))
print('F1-score: ',torch.mean(f1))
